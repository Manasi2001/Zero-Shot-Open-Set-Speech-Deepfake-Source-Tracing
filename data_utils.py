...
class Dataset_Custom(Dataset):
    ...
    def plot_duration_histograms(self, save_dir='duration_plot', filename='duration_histogram.png'):
    # Create directory if it doesn't exist
        os.makedirs(save_dir, exist_ok=True)
        
        # Create and save histogram
        plt.figure(figsize=(10, 4))
        plt.hist(np.array(self.original_durations)/16000, bins=50, alpha=0.5, label='Original (s)')
        plt.hist(np.array(self.trimmed_durations)/16000, bins=50, alpha=0.5, label='Trimmed (s)')
        plt.xlabel('Duration (seconds)')
        plt.ylabel('Count')
        plt.title('Audio Durations Before and After Trimming')
        plt.legend()
        plt.tight_layout()
        
        save_path = os.path.join(save_dir, filename)
        plt.savefig(save_path)
        plt.close()  # Avoid displaying it in non-interactive environments