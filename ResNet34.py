...
class Resnet34Model(nn.Module):
    def __init__(self, device, input_channels=1, input_dim=(100, 80), num_classes=2):
        super(Resnet34Model, self).__init__()
        
        self.resnet34 = vismodel.resnet34(weights=None)
        self.resnet34.conv1 = nn.Conv2d(input_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.resnet34.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.feature_extractor = nn.Sequential(*list(self.resnet34.children())[:-1])

        # Fully connected layers
        self.fc_layers = nn.Sequential(
            nn.Linear(512, 256),  # Feature extraction layer
            nn.ReLU()
        )
        self.classifier = nn.Linear(256, num_classes)  # Classification layer

    def forward(self, x):
        features = self.feature_extractor(x)  # Get feature maps
        features = torch.flatten(features, 1)  # Flatten to (batch_size, 512)
        
        feature_256 = self.fc_layers(features)  # Extract 256-dim feature
        logits = self.classifier(feature_256)  # Final classification output
        
        return logits, feature_256
