...
class Model(nn.Module):
    def __init__(self, args, device):
        ...
        self.out_layer = nn.Linear(5 * gat_dims[1], 9)

    def forward(self, x, y=None):
        # 1. Extract SSL Features
        x_ssl_feat = self.ssl_model.extract_feat(x.squeeze(-1))

        # 2. Apply Mixup (If enabled and training)
        mixup_lambda = None
        target_reordered = None

        if self.apply_mixup and self.training and y is not None:
            mixup_lambda = np.random.beta(self.mixup_alpha, self.mixup_alpha)
            batch_size = x_ssl_feat.size(0)
            index = torch.randperm(batch_size).to(self.device)
            
            # Mix the features
            x_ssl_feat = mixup_lambda * x_ssl_feat + (1 - mixup_lambda) * x_ssl_feat[index, :]
            
            # Store shuffled targets for loss calculation later
            target_reordered = y[index]
        ...
        # Returns: Embedding, Mixup_Lambda, Reordered_Targets
        return last_hidden, mixup_lambda, target_reordered
